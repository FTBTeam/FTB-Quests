		...
		if (questScreen.file.canEdit() && button.isRight()) {
			...
			if (!selected.isEmpty()) {
				if (!selected.contains(quest)) {
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("ftbquests.gui.add_dependencies"),
							ThemeProperties.ADD_ICON.get(),
							() -> selected.forEach(q -> editDependency(quest, q, true)))
					);
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("ftbquests.gui.remove_dependencies"),
							ThemeProperties.DELETE_ICON.get(),
							() -> selected.forEach(q -> editDependency(quest, q, false)))
					);
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("ftbquests.gui.add_dependencies_self"),
							ThemeProperties.ADD_ICON.get(),
							() -> selected.forEach(q -> editDependency(q, quest, true)))
					);
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("ftbquests.gui.remove_dependencies_self"),
							ThemeProperties.DELETE_ICON.get(),
					...
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("ftbquests.gui.add_reward_all"),
							ThemeProperties.ADD_ICON.get(quest),
							this::openAddRewardContextMenu));
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("ftbquests.gui.clear_reward_all"),
							ThemeProperties.CLOSE_ICON.get(quest),
							() -> selected.forEach(q -> q.rewards.forEach(r -> new DeleteObjectMessage(r.id).sendToServer()))));
					contextMenu.add(new ContextMenuItem(new TranslatableComponent("selectServer.delete"),
