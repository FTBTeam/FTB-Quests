		...
		if (button.isLeft()) {
			...
			if (task instanceof ItemTask itemTask) {
				...
				if (!tags.isEmpty() && !ItemFiltersAPI.isFilter(itemTask.item)) {
					builder.insertAtTop(List.of(new ContextMenuItem(new TranslatableComponent("ftbquests.task.ftbquests.item.convert_tag"),
	...
	private class TagSelectionScreen extends ButtonListBaseScreen {
		...
		public void addButtons(Panel panel) {
			for (ResourceLocation tag : tags) {
				panel.add(new SimpleTextButton(panel, new TextComponent(tag.toString()), Icon.EMPTY) {
					@Override
					public void onClicked(MouseButton button) {
						questScreen.openGui();
