...
public class VisualPresets {
    ...
    public static final Codec<Map<String, VisualPresetRecord>> CODEC = Codec.unboundedMap(Codec.STRING, VisualPresetRecord.CODEC);
    ...
    public Json5Element serialize() {
        return CODEC.encodeStart(Json5Ops.INSTANCE, presetMap)
                .resultOrPartial(e -> FTBQuests.LOGGER.error("can't serialize presets! {}", e))
                .orElseGet(Json5Object::new);
    }

    public static VisualPresets deserialize(Json5Element tag) {
        return CODEC.parse(Json5Ops.INSTANCE, tag)
