...
public class EditModeCommand {
    ...
    private static int editingMode(CommandSourceStack source, ServerPlayer player, @Nullable Boolean editMode) {
        return ServerQuestFile.getInstance().getTeamData(player).map(data -> {
            boolean newEditMode = Objects.requireNonNullElse(editMode, !data.isPlayerInEditMode(player));

            data.setPlayerEditMode(player, newEditMode);
