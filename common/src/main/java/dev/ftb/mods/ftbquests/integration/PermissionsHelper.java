...
public class PermissionsHelper {
    ...
    public static boolean hasEditorPermission(CommandSourceStack source) {
        ...
        return source.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER)
                || source.isPlayer() && PermissionHelper.getProvider().getBooleanPermission(source.getPlayer(), EDITOR_PERM, false);
    }
    ...
    public static boolean hasEditorPermission(Player player) {
        return player.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER)
                || player instanceof ServerPlayer sp && PermissionHelper.getProvider().getBooleanPermission(sp, EDITOR_PERM, false);
    }
     ...
     */
    public static boolean canPlayerEdit(PacketContext context) {
        return canPlayerEdit(context.player());
    }
     ...
     * @return true if the player is a valid editor right now
     */
    public static boolean canPlayerEdit(@Nullable Player player) {
        return player != null
                && hasEditorPermission(player)
                && FTBQuestsAPI.api().getQuestFile(false).getTeamData(player)
