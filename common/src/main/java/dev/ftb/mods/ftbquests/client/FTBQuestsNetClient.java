...
import dev.ftb.mods.ftbquests.api.event.progress.*;
...
public class FTBQuestsNetClient {
	...
	public static void createOtherTeamData(TeamDataChangedMessage.TeamDataUpdate dataUpdate) {
	...
	public static void teamDataChanged(TeamDataChangedMessage.TeamDataUpdate dataUpdate) {
		if (ClientQuestFile.exists()) {
			TeamData data = new TeamData(dataUpdate.uuid(), false, dataUpdate.name());
	...
	public static void syncEditingMode(UUID teamId, boolean editingMode) {
		if (ClientQuestFile.getInstance().getOrCreateTeamData(teamId).setPlayerEditMode(ClientUtils.getClientPlayer(), editingMode)) {
	...
	public static void objectStarted(UUID teamId, long id, @Nullable Date time) {
		...
		fireProgressEvent(teamData, id, time, ProgressType.STARTED);
	}
	...
	public static void objectCompleted(UUID teamId, long id, @Nullable Date time) {
		...
		fireProgressEvent(teamData, id, time, ProgressType.COMPLETED);
	}

	private static void fireProgressEvent(TeamData teamData, long id, @Nullable Date time, ProgressType progressType) {
		switch (ClientQuestFile.getInstance().get(id)) {
			case Quest q -> {
				ProgressEventData<Quest> eventData = ProgressEventData.forClient(time, teamData, q);
				NativeEventPosting.get().postEvent(new QuestProgressEvent.Data(progressType, eventData));
			}
			case Chapter c -> {
				ProgressEventData<Chapter> eventData = ProgressEventData.forClient(time, teamData, c);
				NativeEventPosting.get().postEvent(new ChapterProgressEvent.Data(progressType, eventData));
			}
			case Task t -> {
				ProgressEventData<Task> eventData = ProgressEventData.forClient(time, teamData, t);
				NativeEventPosting.get().postEvent(new TaskProgressEvent.Data(progressType, eventData));
			}
			case BaseQuestFile f -> {
				ProgressEventData<BaseQuestFile> eventData = ProgressEventData.forClient(time, teamData, f);
				NativeEventPosting.get().postEvent(new FileProgressEvent.Data(progressType, eventData));
			}
			case null, default -> {}
		}
	}
